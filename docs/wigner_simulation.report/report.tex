...
-\frac{\nabla^2}{2} + V(\mathbf{r}) + \frac{g_{11}}{2} \lvert \psi(\mathbf{r}) \rvert^2 \right) \psi dV \]
...
\[ i \hbar \frac{\partial \psi_1}{\partial t} = \left(
	...
	g_{11} \lvert \psi_1 \rvert^2 + g_{12} \lvert \psi_2 \rvert^2 - i \hbar \Gamma_1
\right) \psi_1 + \frac{\hbar \Omega}{2}e^{i \omega t} \psi_2, \]
\[ i \hbar \frac{\partial \psi_2}{\partial t} = \left(
	-\frac{\hbar^2\nabla^2}{2m} + V_2(\mathbf{r}) +
	g_{12} \lvert \psi_1 \rvert^2 + g_{22} \lvert \psi_2 \rvert^2 - i \hbar \Gamma_2
...
\begin{equation*}
\begin{split}
\frac{\partial W}{\partial t} = & \int\limits_V d\textbf{r} \left\{ - \frac{\partial}{\partial \psi_1} i \left( - \hat{K}_1 + \Gamma_{11} \left( 1 - \lvert \psi_1 \rvert^2 \right) +
...
we must estimate the value of $\lvert \psi_i \rvert^2$:
\[ \langle \lvert \psi_1 \rvert^2 \rangle = \frac{N}{V} \approx \frac{N}{R_{iTF}^3}, \]
where $R_{iTF}$ is the radius of the condensate, obtained using Thomas-Fermi approximation.
For $10^4$--$10^5$ atoms it gives the values of the order of $10^2$, therefore we can safely assume that:
\begin{equation}
\label{psi_estimation}
\lvert \psi_i \rvert^2 \gg 1
\end{equation}
Areas where it is not true contain negligible amounts of atoms and therefore do not affect the evolution.

Thus we can drop out lower orders of $\lvert \psi_1 \rvert^2$ and $\lvert \psi_2 \rvert^2$, and the following correspondences are valid:
\[
...
Using the same estimation of $\lvert \psi_1 \rvert^2$ and $\lvert \psi_2 \rvert^2$,
...
\begin{equation*}
\begin{split}
\int\limits_V \langle \left\{ \hat{\psi}^r ( \hat{\psi}^\dagger )^s \right\}_{sym} \rangle d\textbf{r} & =
\int\limits_V \int \psi^r \left( \psi^* \right)^s W(\psi, \psi^*) d\psi^2 d\textbf{r} \\
...
\langle \hat{\psi}_i \hat{\psi}^\dagger_i \rangle - 1 \right) \\
& = \langle \left\{ \hat{\psi} \hat{\psi}^\dagger \right\}_{sym} \rangle - \frac{1}{2} \\
& = \frac{1}{N_{paths}} \sum\limits_{paths} \lvert \psi_i \rvert^2 - \frac{1}{2}
\end{split}
\end{equation*}
and population:
\begin{equation*}
\begin{split}
N_i & = \int\limits_V n_i d\textbf{r} \\
& \approx \sum\limits_V \left( \frac{1}{N_{paths}} \sum\limits_{paths} \lvert \psi_i \rvert^2 - \frac{1}{2} \right) dV \\
& = \frac{1}{N_{paths}} \sum\limits_{paths} \sum\limits_{V} \lvert \psi_i \rvert^2 dV - \frac{N_{cells} dV}{2},
\end{split}
\end{equation*}
where $N_{cells}$ is the number of cells in spatial lattice.

Interaction term $\langle \hat{\psi}^\dagger_1 \hat{\psi}_2 \rangle$, which is necessary for visibility measurement,
can serve as another example. Operators $\hat{\psi}_1$ and $\hat{\psi}_2$ do not commute,
therefore their symmetrically ordered product is equal to the normally ordered product, which gives us the simple formula:
\[
\int\limits_V \langle \hat{\psi}^\dagger_1 \hat{\psi}_2 \rangle d\textbf{r} =
\frac{1}{N_{paths}} \sum\limits_{paths} \sum\limits_{V} \psi_1^* \psi_2 dV
\]
