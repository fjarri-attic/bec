...
Next in line is a visualisation technique, proposed by Russell Anderson. The idea is to project density of each state to $z$-axis and render the difference between the density of two states along this axis in time. In order to avoid meaningless noise at the borders of the condensate (where the density is low), we are dividing density difference by average axial density (which equals to total number of atoms divided by simulated volume). In addition to noise elimination, it keeps the absolute value of the difference, so the revival/decay is clearly seen on the graph. Fig~\ref{axial_view} shows the example of such visualisation for detuning $\Delta = -41 \textrm{ Hz}$.
