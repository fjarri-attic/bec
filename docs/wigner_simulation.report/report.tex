...
\left( \frac{\hbar \Omega}{2}e^{i \omega t} + \textrm{c.c.} \right) \psi_2 \\
i \hbar \frac{\partial \psi_2}{\partial t} & = \left( - \frac{\hbar^2 \nabla^2}{2 m} + V_2 + V_{hf} +
g_{22} \lvert \psi_2 \rvert^2 + g_{12} \lvert \psi_1 \rvert^2 - i \hbar \Gamma_2 \right) \psi_2 +
\left( \frac{\hbar \Omega}{2}e^{i \omega t} + \textrm{c.c.} \right) \psi_1
\end{split}
\end{align}
...
Coupling coefficient $\Omega$ is a complex number $\lvert \Omega \rvert e^{i \alpha}$,
where $\lvert \Omega \rvert$ is the Rabi frequency (its exact value depends on the nature of coupling process),
...
which is significant when simulating two-pulse Ramsey interferometry.
Transformed equations look like:
\begin{align*}
\begin{split}
i \hbar \frac{\partial \psi_1}{\partial t} = & \left( - \frac{\hbar^2 \nabla^2}{2 m} + V_1 +
g_{11} \lvert \psi_1 \rvert^2 + g_{12} \lvert \psi_2 \rvert^2 - i \hbar \Gamma_1 \right) \psi_1 + \\
& + \left( \frac{\hbar \Omega}{2}e^{i (\omega + \omega_{hf}) t} + \frac{\hbar \Omega^*}{2}e^{-i \delta t} \right) \psi_2 \\
i \hbar \frac{\partial \psi_2}{\partial t} = & \left( - \frac{\hbar^2 \nabla^2}{2 m} + V_2 +
g_{22} \lvert \psi_2 \rvert^2 + g_{12} \lvert \psi_1 \rvert^2 - i \hbar \Gamma_2 \right) \psi_2 + \\
& + \left( \frac{\hbar \Omega}{2}e^{i \delta t} + \frac{\hbar \Omega^*}{2}e^{-i (\omega + \omega_{hf})} \right) \psi_2
\end{split}
\end{align*}
...
i \hbar \frac{\partial \psi_1}{\partial t} & = \left( - \frac{\hbar^2 \nabla^2}{2 m} + V_1 +
g_{11} \lvert \psi_1 \rvert^2 + g_{12} \lvert \psi_2 \rvert^2 - i \hbar \Gamma_1 \right) \psi_1 +
\frac{\hbar \lvert \Omega \rvert}{2}e^{-i \delta t - \alpha} \psi_2 \\
i \hbar \frac{\partial \psi_2}{\partial t} & = \left( - \frac{\hbar^2 \nabla^2}{2 m} + V_2 +
g_{22} \lvert \psi_2 \rvert^2 + g_{12} \lvert \psi_1 \rvert^2 - i \hbar \Gamma_2 \right) \psi_2 +
\frac{\hbar \lvert \Omega \rvert}{2}e^{i \delta t + \alpha} \psi_2,
\end{split}
\end{align}
where $\alpha$ is the phase of the coupling field at the beginning of the pulse.
When pulse is applied twice using the same coupling field (which is the case for Ramsey interferometry),
phases are dependent: $\alpha_2 = \alpha_1 + \omega t_{R}$, where $t_{R}$ is the Ramsey time.

The application of the coupling field can be simplified, if certain additional conditions are valid, namely:
\begin{enumerate}
\item $\mu / \hbar \ll \lvert \Omega \rvert$, where $\mu$ is the chemical potential of the first component;
\item $\delta \ll \lvert \Omega \rvert$;
...
\end{pmatrix} =
\begin{pmatrix}
	\cos \frac{\theta}{2} & -i e^{-i \alpha} \sin \frac{\theta}{2} \\ -i e^{i \alpha} \sin \frac{\theta}{2} & \cos \frac{\theta}{2}
\end{pmatrix}
\begin{pmatrix}
	\psi_1 \\ \psi_2
\end{pmatrix},
\end{equation}
where $\theta = \Omega t_{pulse}$.

For example, matrix for $\frac{\pi}{2}$-pulse, which is used in Ramsey interferometry, will look like:
\[
\begin{pmatrix}
	\psi^\prime_1 \\	\psi^\prime_2
\end{pmatrix} =
\frac{1}{\sqrt{2}} \begin{pmatrix}
	1 & -i e^{-i \alpha} \\ -i e^{i \alpha} & 1
\end{pmatrix}
\begin{pmatrix}
	\psi_1 \\ \psi_2
\end{pmatrix}.
\]
Coupling field phase $\alpha$ can be taken to be equal to zero for the first pulse and $\omega t_{R}$ for the second pulse.
