...
\]
which look almost like coupled GPEs, except for the phase shifting term. Interaction coefficients $\Gamma_{11}$, $\Gamma_{12}$ and $\Gamma_{22}$ can be identified as $g_{11}$, $g_{12}$ and $g_{22}$ from GPEs.
...
\end{split}
\end{equation*}
where $\hat{A}_1$ and $\hat{A}_2$ stand for previously calculated kinetic, potential and interaction terms for both components. In order to connect $\kappa$ coefficients with loss coefficients $\gamma$ in GPEs, we must consider $\vert \psi_i \vert^2 \gg 1$ (areas where it is not true, contain negligible amounts of atoms and therefore do not affect the evolution). Really, the average value of $\vert \psi_1 \vert^2$ can be estimated as following:
\[
\bar{\vert \psi_1 \vert^2} = \frac{N}{V} \approx \frac{N}{R_{TF}^3},
\]
where $R_{TF}$ is the radius of the condensate, obtained using Thomas-Fermi approximation. For $10^4$--$10^5$ atoms it gives the values of the order of $10^2$, which proves that our initial assumption was correct. Therefore, we can drop out lower orders of $\vert \psi_1 \vert^2$ and $\vert \psi_2 \vert^2$, and the following correspondences are valid:
\[
