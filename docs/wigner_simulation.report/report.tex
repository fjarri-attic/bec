...
The evolution of the system is described by coupled Gross-Pitaevskii equations. They can contain different
terms depending on which features of the system we need to take into account. Basic form can be found in
many sources, i.e. \cite{pitaevskii_bec}; with the addition of loss terms it looks like:
...
Here $V_1$ and $V_2$ stand for the external potential (which, in general, is different for each component),
$g_{11}$, $g_{12}$ and $g_{22}$ are interaction coefficients, $\Gamma_1$ and $\Gamma_2$ are responsible for
two- and three-body loss processes correspondingly, $\Omega$ is a constant for state transitions due to
external oscillating magnetic field, and $\Delta$ is a detuning frequency.
